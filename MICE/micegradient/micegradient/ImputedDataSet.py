...
class ImputedDataSet(_ImputationSchema):
    ...
    def __init__(
        ...
        initialization: str = 'median',
        imputed_dataset: DataFrame = None
    ):
        ...
        # random draws is one of the inilization option instead of being the only one #ref-10132021
        ...
        # modified to add median initialization #ref-101321
        elif initialization == 'median':
            for var in self._all_imputed_vars:
                self.imputation_values[var] = {
                    0: np.full(shape=self.na_counts[var],fill_value=data[var].median(),dtype=np.float_)
                }
        elif initialization == 'recluster':
            for var in self._all_imputed_vars:
                self.imputation_values[var] = {0: imputed_dataset[self.na_where][var].dropna().to_numpy()}
