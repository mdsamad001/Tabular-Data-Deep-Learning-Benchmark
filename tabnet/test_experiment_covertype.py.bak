...
def main(unused_argv):
  ...
  print('creating train/val/test iterators')
  ...
  print('Define the model and losses')
  ...
  with tf.Session() as sess:
    ...
    pbar.set_description('running init.')
    sess.run(init)
    pbar.set_description('running init_local.')
    sess.run(init_local)
    pbar.set_description('running init_table.')
    sess.run(init_table)
    pbar.set_description('running train_init.')
    sess.run(train_iter.initializer)
    pbar.set_description('running val_init.')
    sess.run(val_iter.initializer)
    pbar.set_description('running test_init.')
    ...
    for step in pbar:
      if step % DISPLAY_STEP == 0:
        ...
        pbar.set_description("Step " + str(step) + " , Training Loss = " +
      ...
      if step % VAL_STEP == 0:
        ...
        pbar.set_description("Step " + str(step) + " , Val Accuracy = " +
              "{:.4f}".format(val_acc))
        summary_writer.add_summary(merged_summary, step)
