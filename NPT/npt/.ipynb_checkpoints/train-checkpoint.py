...
from NPT.npt.column_encoding_dataset import ColumnEncodingDataset, NPTDataset
from NPT.npt.loss import Loss
from NPT.npt.optim import LRScheduler
from NPT.npt.optim import TradeoffAnnealer
from NPT.npt.utils import debug
from NPT.npt.utils.batch_utils import collate_with_pre_batching
from NPT.npt.utils.encode_utils import torch_cast_to_dtype
from NPT.npt.utils.eval_checkpoint_utils import EarlyStopCounter, EarlyStopSignal
from NPT.npt.utils.logging_utils import Logger
...
class Trainer:
    ...
    def get_num_steps_per_epoch(self):
        ...
        print('N', N,';','num_steps_per_epoch', int(np.ceil(N / self.c.exp_batch_size)))
    ...
    def eval_model(self, train_loss, epoch, end_experiment):
        ...
        if ((counter == EarlyStopSignal.STOP)
            ...
            self.loss.last_run = True
            self.loss.y_pred, self.loss.y_true = [],[]
            test_loss = self.run_epoch(dataset_mode='test', **kwargs)   
            
            # if end_experiment:
            #     print('---End expt test eval--')
            #     print(test_loss)
            #     print()
