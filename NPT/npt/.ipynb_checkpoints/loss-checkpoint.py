...
class Loss:
    ...
    * Loss.finalize_epoch_losses() to obtain the loss_dict for the current
    ...
    def compute_loss(
        ...
        for col, (out, dat) in enumerate(zip(output, ground_truth_data)):
            ...
            for mode, mode_loss_indices in loss_indices.items():
                ...
                # if col==0: ## do for target cols only
                #     print('---compute loss---')
                #     print('mode', mode)
                #     print('total', num_preds)
                #     print(mode_loss_indices.shape)
                #     print(mode_loss_indices)
                #     print('---------')
    ...
    def balance_self_supervision(self, raw_dict):
        ...
        """
        tradeoff_setting = self.c.exp_tradeoff
        ...
        if tradeoff_setting == -1:
            ...
            label_loss = (
                ...
                (raw_dict['label']['cat_total_preds'] +
                 raw_dict['label']['num_total_preds']))
            if self.c.model_augmentation_bert_mask_prob[
                    self.dataset_mode] == 0:
                aug_loss = 0
            else:
                aug_loss = (
    ...
    def compute_column_loss(
        ...
        if is_cat:
            ...
            if eval_model:
                ...
                if self.dataset_mode == 'test' and self.last_run:
                    # print('---per_column_loss/eval_model---')
                    # print('valid_long_output', valid_long_output)
                    # print('valid_long_data', valid_long_data)
                    # print('acc', cat_correct_preds/valid_long_output.shape[0])
                    # print('--- end ---')
                    self.y_pred.extend(valid_long_output.cpu().tolist())
                    self.y_true.extend(valid_long_data.cpu().tolist())
