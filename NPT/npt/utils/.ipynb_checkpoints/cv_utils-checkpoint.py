...
def get_class_reg_train_val_test_splits(
    ...
    if c.exp_bootstrap > 0:
        idx = np.arange(N)
        train_test_splits = [
        train_test_split(
                idx, test_size=c.exp_test_perc, shuffle=True,
                random_state=i, stratify=label_rows) for i in range(c.exp_bootstrap)
        ]
...
def get_n_cv_splits(c):
    return int(1 / c.exp_test_perc) if c.exp_bootstrap < 0 else c.exp_bootstrap  # Rounds down otherwise return no. of bootstraps
