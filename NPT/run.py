...
from NPT.baselines.sklearn_tune import run_sklearn_hypertuning
from NPT.npt.column_encoding_dataset import ColumnEncodingDataset
from NPT.npt.configs import build_parser
from NPT.npt.distribution import distributed_train_wrapper
from NPT.npt.train import Trainer
from NPT.npt.utils.model_init_utils import init_model_opt_scaler_from_dataset
from NPT.npt.utils.viz_att_maps import viz_att_maps
...
def main(args):
    ...
    print('data_set_on_cuda',args.data_set_on_cuda)
    
    fold_preds, fold_trues = run_cv(args=args, wandb_args=wandb_args)
...
def run_cv(args, wandb_args):
    ...
    if c.model_class == 'NPT':
        fold_preds, fold_trues = run_cv_splits(wandb_args, args, c, wandb_run)
...
def run_cv_splits(wandb_args, args, c, wandb_run):
    ...
    for cv_index in range(starting_cv_index, total_n_cv_splits):
        ...
        #######################################################################
        # Run training
        # print('n_batches', dataset.n_batches)
        ...
        y_pred = trainer.loss.y_pred#.cpu().numpy()
        y_true = trainer.loss.y_true#.cpu().numpy()
