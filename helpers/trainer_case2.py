...
def model_pretraining(model, train_x, valid_x,
    ...
    # train_x = torch.concat([train_x, valid_x])
...
#         tries -= 1  # reduce tries after validation
        ...
        mean_val_loss = 0 # zero as placeholder
...
#         tries -= 1
        ...
        mean_val_loss = 0 #np.sum(batch_val_losses) / valid_samples
...
#             tries = patience
        ...
        y_train, y_test = [x.cpu().detach().numpy() for x in [
            train_y, test_y]]
        ...
        valid_scores.append(0) # send it or it breaks
...
def ae_training(dataset_name, batch_size,
    ...
    for fold_counter in range(fold_limit):
        ...
        train_x, test_x = [
            torch.from_numpy(x.values).float().to(device) for x in [train_df, test_df]
        ]
        train_y, test_y = [
            torch.from_numpy(x).long().to(device) for x in [train_y, test_y]
        ]
        ...
        valid_score, _, _ = (0, 0, 0) #get_score(valid_x, valid_y)
