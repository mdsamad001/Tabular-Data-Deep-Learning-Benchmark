...
from helpers.openml_data_v2 import get_data, get_data1, get_data_from_featllm
...
def ae_training(dataset_name, batch_size,
    ...
    ### llm data preprocess
    df,y, cats = get_data_from_featllm(dataset_name)
    
    for c, m in zip(df.columns, cats):
        if not m:
            continue
    ...
    for fold_counter in range(fold_limit):
        ...
        torch.save(pretrained_model,
                   f'{save_path}_pretrained-model_{fold_counter}.pkl')
        ...
        def get_score(current_model, X_gpu, y_gpu):
            ...
            y_pred_probs = current_model(preprocessor.transform(
                X).float().to(device)).softmax(dim=1).cpu().detach().numpy()
            y_pred = y_pred_probs.argmax(1)
            score = f1_score(y_true, y_pred, average='weighted')
            
            auc = utils_evaluate(y_pred_probs, y_true, multiclass=y_pred_probs.shape[1]>2)
            ...
            return score, auc, y_true, y_pred_probs

        valid_score, valid_auc, _, _ = get_score(model, valid_x, valid_y)
        test_score, test_auc, y_true, y_pred = get_score(model, test_x, test_y)

        final_valid_score_fold.append(valid_score)
        final_test_score_fold.append(test_score)
        final_test_auc_fold.append(test_auc)
    ...
    return (
        ...
        curve_list, latent, final_test_score_fold, final_test_auc_fold,
        ...
        test_actuals, test_predictions, # predicted probs
        z_tests, z_test_scores,  # sklearn model scores on z
        p_tests, p_test_scores,  # sklearn model scores on p
        fold_pretrain_times,
    )

def utils_evaluate(pred_probs, answers, multiclass=False):   
    if multiclass == False:
...
def do_all(dataset_name='breast_cancer', batch_size=128,
    ...
    (pretraining_loss_list, finetuning_loss_list,
     curve_list, basic_latent, fold_scores, fold_aucs, fold_curves,
...
#     shaded_plot(BFT_Error, "Basic AE Finetuning", 'Epoch', 'Loss', save_path + 'basic_finetuning.pdf')
    ...
    return fold_scores, fold_aucs, test_actuals, test_predictions, z_test_scores, p_test_scores, batch_times, time_taken
