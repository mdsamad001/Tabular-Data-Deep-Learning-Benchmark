...
default_settings = {
    ...
    'n_neg_pairs': 128,
    'train_workers': 0,
    'redo_pretrain': True, # keep it True; Loading each fold's pretrained state model is not properly coded. TODO: fix
}
...
def model_pretraining(model, train_x, valid_x,
    ...
    def corrupt_batch(sample_list):
        batch = torch.stack(sample_list).float()
        batch1 = preprocessor1.transform(batch).float()
        batch2 = preprocessor2.transform(batch).float()

        return batch, batch1, batch2


    data_loader = torch.utils.data.DataLoader(dataset=train_x.to('cpu'),
...
def ae_training(dataset_name, batch_size,
    ...
    for fold_counter in range(fold_limit):
        ...
        try:
            ...
            pbar and pbar.set_description(f'fold {fold_counter} model not loaded')
            
        # batch_times will not appended if model was loaded
        if not model_loaded or settings['redo_pretrain']:
            pretrained_model, training_lle, latent, batch_times = model_pretraining(pretrain_model,
                                                                                train_x, valid_x,
                                                                                cols, cats, col_preprocessor,
                                                                                unsupervised_epochs, batch_size)
            torch.save(pretrained_model.state_dict(), pretrain_model_state_path)
