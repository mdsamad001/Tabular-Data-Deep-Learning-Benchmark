...
from helpers.openml_data_v2 import get_data
...
def model_pretraining(model, dataset, valid_x, 
    ...
    for epoch in range(1, num_of_epoch+1):
        ...
        for batch_idx,train in enumerate(data_loader):
            ...
            train = train.float()
            train1 = corruptor1(train)
            # if settings['corruptor1']['method'] == 'pass':
            #     assert torch.equal(train1, train)
            train2 = corruptor2(train)
            
            assert train.shape[1] == train1.shape[1], 'corrupted feature size different'
