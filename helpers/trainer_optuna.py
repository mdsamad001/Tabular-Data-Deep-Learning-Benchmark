...
def model_pretraining(model, train_x, valid_x,
    ...
    if missing_rate >= 0:
        settings_copy['corruptor1'].update({'missing': missing_rate})
        settings_copy['corruptor2'].update({'missing': missing_rate})
    
    # for training corruption
    preprocessor1 = get_fitted_pipeline(train_x, cols, cats, col_preprocessor,
                                        settings_copy['corruptor1'],
                                        settings_copy['corrupt_before_ohe'])
    preprocessor2 = get_fitted_pipeline(train_x, cols, cats, col_preprocessor,
                                        settings_copy['corruptor2'],
                                        settings_copy['corrupt_before_ohe'])
    ...
    for epoch in range(1, num_of_epoch+1):
        ...
        pbar and pbar.set_description((
            f'Pretraining (m={missing_rate:.2f}) | epoch: {epoch}/{num_of_epoch}; '
...
def model_finetuning(model, preprocessor,
    ...
    return model, min_loss, [finetuning_loss, finetuning_val_loss], [train_scores, valid_scores, test_scores]
...
def ae_training(dataset_name, batch_size,
    ...
    for fold_counter in range(fold_limit):
        ...
        # return best val score (cross-entropy) from finetuning
        # we do not want to use val_loss from pretraining.
        # because lower missing rate will always give smaller loss value
        def objective(trial):
            missing_rate = trial.suggest_float("m", 0, 1)
            ...
            model, best_val_score, finetuning_loss, scores_list = model_finetuning(model,preprocessor,
        ...
        best_missing = study.best_params['m']
        
        print('best_missing=', best_missing)
        best_missing_fold.append(best_missing)
        ...
        # finetuning done here
        # note: finetuning_loss = (train_loss, valid_loss)
        model, _, finetuning_loss, scores_list = model_finetuning(model,preprocessor,
                                                        supervised_epochs, batch_size,
                                                        train_x, train_y,
                                                        valid_x, valid_y,
                                                        test_x, test_y
    ...
    return (
        ...
        test_actuals, test_predictions, best_missing_fold
    )
...
def do_all(dataset_name='breast_cancer', batch_size=128,
    ...
    (pretraining_loss_list, finetuning_loss_list,
     ...
     test_actuals, test_predictions, best_missing_folds) = ae_training(dataset_name,
                                                   batch_size=batch_size,
    ...
    info_dict = {
        ...
        'best_missing': best_missing_folds,
...
#     shaded_plot(BFT_Error, "Basic AE Finetuning", 'Epoch', 'Loss', save_path + 'basic_finetuning.pdf')
    ...
    return fold_scores, best_missing_folds
