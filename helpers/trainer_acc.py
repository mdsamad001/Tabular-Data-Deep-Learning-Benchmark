...
def model_finetuning(model, preprocessor,
    ...
    for epoch in range(1, num_of_epoch+1):
        ...
        for batch_idx, (valid, target) in enumerate(valid_data_loader):
            ...
            y_preds += list(pred_valid.argmax(1).cpu().detach().numpy())
            y_trues += list(target.long().cpu().numpy())
        ...
        pbar and pbar.set_description((
            ...
            f'mean_loss: {mean_loss:.3f} ; '
            f'val_acc: {val_acc:.3f} (best: {max_val_acc:.3f}); '
            f'tries = {tries}'
        ))
...
def ae_training(dataset_name, batch_size,
    ...
    for fold_counter in range(fold_limit):
        ...
        torch.save(pretrained_model, f'{save_path}_pretrained-model_{fold_counter}.pkl')
        ...
        if settings['test_pretrain']:
            ...
            with torch.no_grad():
                train_z, train_p = pretrained_model(preprocessor.transform(train_x).float().to(device))
                valid_z, valid_p = pretrained_model(preprocessor.transform(valid_x).float().to(device))
                test_z, test_p = pretrained_model(preprocessor.transform(test_x).float().to(device))
            ...
            for baseline_model in baselines:
                ...
                baseline_score, best_params, y_pred = train_and_test(clf, params, 
                                                                     (train_z.cpu().numpy(), train_y.cpu().numpy()), 
                                                                     (valid_z.cpu().numpy(), valid_y.cpu().numpy()), 
                                                                     (test_z.cpu().numpy(), test_y.cpu().numpy()),
                                                                     pbar=pbar, n_jobs=12,
                                                                    )
                ...
                baseline_score, best_params, y_pred = train_and_test(clf, params, 
                                                                     (train_p.cpu().numpy(), train_y.cpu().numpy()), 
                                                                     (valid_p.cpu().numpy(), valid_y.cpu().numpy()), 
                                                                     (test_p.cpu().numpy(), test_y.cpu().numpy()),
                                                                     pbar=pbar, n_jobs=12,
                                                                    )
        ...
        def get_score(current_model, X_gpu, y_gpu):
            ...
            y_pred = current_model(preprocessor.transform(X).float().to(device)).cpu().detach().numpy().argmax(1)
