...
def model_pretraining(model, train_x, train_y, 
    ...
    for epoch in range(1, num_of_epoch+1):
        ...
        model = model.train()
        for batch_idx, (train, train1, train2) in enumerate(data_loader):
            ...
            with torch.autograd.set_detect_anomaly(False): # disable for some slight time improvements
                ...
                t_start = time()
                loss = pretrain_loss(z1, z2)  # contrastive loss
        ...
        for batch_idx, (valid1, valid2) in enumerate(valid_data_loader):
            ...
            val_loss = pretrain_loss(z1, z2)  # contrastive loss
...
def ae_training(dataset_name, batch_size,
    ...
    for fold_counter in range(fold_limit):
        ...
        if settings['fs']!=-1:
            # print(train_x.shape)
            use_only = int(train_x.shape[1] / settings['fs'])
            train_x = train_x[:use_only] # reduce training samples
            train_y = train_y[:use_only]
            # print(train_x.shape)
