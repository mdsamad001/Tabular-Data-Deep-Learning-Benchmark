...
default_settings = {
    ...
    # 'patience': 3, # wait for 3 epochs to improve validation score
    'corruptor1': {
        'method': 'pass',
    },
    'corruptor2': {
...
def model_pretraining(model, dataset, valid_x, 
    ...
    corruptor1 = Corruptor(dataset, settings['corruptor1'])
    corruptor2 = Corruptor(dataset, settings['corruptor2'])
    # for validation corruption
    valid_corruptor1 = Corruptor(valid_x, settings['corruptor1'])
    valid_corruptor2 = Corruptor(valid_x, settings['corruptor2'])
    ...
    for i in range(10):
        corupted_valid_x1 = valid_corruptor1(valid_x)
        # if settings['corruptor1']['method'] == 'pass':
        #     assert torch.equal(corrupted_valid_x1, valid_x)
        corupted_valid_x2 = valid_corruptor2(valid_x)
        # add to list after shuffling the rows
        shuffled_idx = torch.randperm(valid_x.shape[0])
        valid_dataset = data_utils.TensorDataset(corupted_valid_x1, corupted_valid_x2)
    ...
    optimizer = torch.optim.Adam(model.parameters(), lr=1e-3) #lr=1e-5, weight_decay=1e-5 
    ...
    for epoch in range(1, num_of_epoch+1):
        ...
        # modified to do corruption at start of epoch
        corrupted_train1 = corruptor1(dataset)
        corrupted_train2 = corruptor2(dataset)
        corrupted_dataset = data_utils.TensorDataset(corrupted_train1, corrupted_train2)
        data_loader = data_utils.DataLoader(dataset=corrupted_dataset,
                                                  batch_size=batch_size,
                                                  shuffle=True)
        
        model = model.train()
        for batch_idx,(train1, train2) in enumerate(data_loader):
            assert train1.shape[1] == train2.shape[1], 'corrupted feature size different'
            ...
            batch_losses.append(loss.cpu().item() * train1.shape[0])
            train_samples += train1.shape[0]
        ...
        mean_val_loss = np.sum(batch_val_losses) / valid_samples #val_loss_per_sample
        ...
        # if validation loss is decreasing before stopping
        if tries<=0: break            
...
def model_finetuning(model, train_tensor, 
                     num_of_epoch, batch_size, 
                     train_x, train_y, 
                     valid_x, valid_y,
                     test_x, test_y):
    ...
    data_loader = data_utils.DataLoader(dataset=train_tensor,
                                        batch_size=batch_size,
                                        shuffle=True)
    ...
    for epoch in range(1,num_of_epoch+1):
        ...
        for batch_idx, (train, target) in enumerate(data_loader):
            ...
            # print('predition, target = ', prediction.shape, target.shape, target.unsqueeze(1).shape)
            ...
            batch_losses.append(classification_loss.cpu().item() * train.shape[0])
        ...
        for batch_idx, (valid, target) in enumerate(valid_data_loader):
            ...
            valid_loss = criterion(pred_valid, valid_y).item()
        ...
        y_train, y_valid, y_test = [x.cpu().detach().numpy() for x in [train_y, valid_y, test_y]]
        
        # train accuracy after each epoch
        pred_train = model(train_x).argmax(1).cpu().detach().numpy()        
        train_accuracy = f1_score(y_train, pred_train, average='weighted')
        train_scores.append(train_accuracy)

        # test accuracy after each epoch
        pred_test = model(test_x).argmax(1).cpu().detach().numpy()
        test_accuracy = f1_score(y_test, pred_test, average='weighted')
        test_scores.append(test_accuracy)
        
        # valid accuracy after each epoch
        pred_valid = model(valid_x).argmax(1).cpu().detach().numpy()
    ...
    return model, [finetuning_loss, finetuning_val_loss], [train_scores, valid_scores, test_scores], prediction


# todo: change to model_training
def ae_training(model_name, dataset, k_folds, batch_size, output_size=1,
    ...
    kfold_splits = list(cv.split(dataset, dataset[y_column_name]))
    
    # run k fold in loop
    for fold_counter in range(fold_limit):
        pbar and pbar.edit_last_prefix(f'fold={fold_counter+1}/{fold_limit} | ')
        ...
        if settings['method'] == 'kfold':
            ...
            train_df, test_df, _, _ = train_test_split(dataset, dataset[y_column_name],
                                             stratify = dataset[y_column_name],
                                             shuffle=True,
                                             test_size=1/5, random_state=fold_counter)
            
        
        train_df, valid_df, _, _ = train_test_split(train_df, train_df[y_column_name], 
                                                    stratify = train_df[y_column_name],
                                                    test_size=1/8, random_state=42)
        

        train_df, valid_df, test_df = standardize_dataset(train_df, [train_df, valid_df, test_df], y_column_name)
        


        # convert dataframe into train and test tensor
        train_x, train_y, train_tensor = convert_df_to_tensor(train_df)
        valid_x, valid_y, valid_tensor = convert_df_to_tensor(valid_df)
        ...
        pretrained_model, training_lle, latent = model_pretraining(pretrain_model, train_x.float(), valid_x.float(),
        ...
        model, finetuning_loss, scores_list, prediction = model_finetuning(model, train_tensor, 
                                                                           supervised_epochs, batch_size, 
                                                                           train_x.float(), train_y.long(), 
                                                                           valid_x.float(), valid_y.long(), 
                                                                           test_x.float(), test_y.long()
                                                                          )
        ...
        valid_score = get_score(valid_x, valid_y)
        test_score  = get_score(test_x, test_y)        
    ...
    classification_accuracy_mean = np.round(statistics.mean(final_test_score_fold),4)
    classification_accuracy_std = np.round(statistics.pstdev(final_test_score_fold),4)
...
def do_all(dataset_name='breast_cancer', batch_size=128, 
    ...
    # data_df = get_data_df(dataset_name)
    X, y = get_data(dataset_name)
    data_df = pd.DataFrame(X)
    data_df[y_column_name] = y
    
    # v2 because different early stopping strat
    ...
    (pretraining_loss_list, finetuning_loss_list, 
     curve_list,basic_latent, fold_scores, fold_curves) = ae_training('basic', data_df, k_folds,
                                                         batch_size=batch_size, 
                                                         output_size = data_df['target'].unique().shape[0],
                                                         unsupervised_epochs=unsupervised_epochs,
                                                         supervised_epochs=supervised_epochs,
                                                         save_path=save_path)
    
    time_taken = time() - starting_time
    print(f'time taken, total={time_taken:.2f}s or {(time_taken/5):.2f}s per fold')
