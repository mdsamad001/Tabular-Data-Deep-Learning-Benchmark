...
def set_random_seeds(x=0):
    torch.manual_seed(x)
    np.random.seed(x)
    random.seed(x)  # mcar sampler uses random maybe
...
def model_pretraining(model, train_x, valid_x,
                      ...
                      num_of_epoch, batch_size, settings,
    ...
    pbar and pbar.set_description('corrupting validation set')
...
def ae_training(dataset_name, batch_size,
    ...
    test_predictions = []
    fold_pretrain_times = []
    fold_counter_list = []  # because parallel can call/finish different fold counter async
    ...
    fold_returns_list = Parallel(n_jobs=n_jobs)(delayed(do_fold)(
        settings, batch_size, unsupervised_epochs, supervised_epochs, df, y, cats, cols,
        fold_limit, kfold_splits, col_preprocessor, fold_counter
    ) for fold_counter in range(fold_limit))

    # fold_returns_list = [do_fold(
    #     settings, batch_size, unsupervised_epochs, supervised_epochs, df, y, cats, cols,
    #     fold_limit, kfold_splits, col_preprocessor, fold_counter
    # ) for fold_counter in range(fold_limit)]

    for fold_returns in fold_returns_list:
        fold_counter = fold_returns[0]
        batch_times = fold_returns[1],
        training_lle = [fold_returns[2], fold_returns[3]]
        finetuning_loss = [fold_returns[4], fold_returns[5]]
        scores_list = [*fold_returns[6:9]]

        fold_counter_list.append(fold_counter)
        ...
        valid_score, test_score, y_true, y_pred = (
            i for i in fold_returns[-4:])
    ...
    return (
        ...
        fold_pretrain_times, fold_counter_list
    )


def do_fold(settings, batch_size, unsupervised_epochs, supervised_epochs,
            df, y, cats, cols,
            fold_limit, kfold_splits, col_preprocessor, fold_counter):
    set_random_seeds(fold_counter)
    ...
    if unsupervised_epochs > 0:
        ...
        pretrained_model, training_lle, latent, batch_times = pretrain_model, [
            [], []], False, [0]
    ...
    def get_score(X_gpu, y_gpu):
        ...
        y_pred = model(preprocessor.transform(X).float().to(
            device)).cpu().detach().numpy().argmax(1)
    ...
    return (
        fold_counter, batch_times,
        *training_lle,  # pretraining loss on train n valid (2 elements)
        *finetuning_loss,  # finetuning loss on train n valid (2 elements)
        *scores_list,  # finetuning scores on train, valid n test (3 elements)
        valid_score, test_score, y_true, y_pred)
...
def do_all(dataset_name='breast_cancer', batch_size=128,
    ...
    (pretraining_loss_list, finetuning_loss_list,
     ...
     p_tests, p_test_scores, batch_times,
     fold_counter_list) = ae_training(dataset_name,
                                      batch_size=batch_size,
    ...
    info_dict = {
        ...
        'fold_counter_list': fold_counter_list,
    }
