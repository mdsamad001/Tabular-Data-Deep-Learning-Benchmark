...
# override the pretraining model
...
def encoder_loss(z1, target=1):
    ...
    mask = torch.triu(torch.ones_like(c), diagonal=1)
    c_triu = c[mask==1]
    loss = (target - c_triu).pow(2).sum()
    
    return loss #/ c_triu.shape[0]
    ...
    for epoch in range(1, num_of_epoch+1):
        ...
        for batch_idx, train in enumerate(data_loader):
            ...
            with torch.autograd.set_detect_anomaly(True):
                ...
                contrastive_loss = pretrain_loss(p1, p2)  # contrastive loss
                encoder_loss1 = encoder_loss(z1)
                encoder_loss2 = encoder_loss(z2)
                
                loss = contrastive_loss + (encoder_loss1 + encoder_loss2) / 2
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
        ...
        for batch_idx, (valid1, valid2) in enumerate(valid_data_loader):
            ...
            val_loss = contrastive_loss + (encoder_loss1 + encoder_loss2) / 2
